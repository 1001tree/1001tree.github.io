...
function loadVue() {
	...
	Vue.component('challenge', {
		...
		<div v-if="tmp[layer].challenges && tmp[layer].challenges[data]!== undefined && tmp[layer].challenges[data].unlocked && !(options.hideChallenges && maxedChallenge(layer, [data]) && !inChallenge(layer, [data]))"
			...
			<span v-else>
				...
				<span v-if="layers[layer].challenges[data].rewardDisplay!==undefined">效果: <span v-html="(tmp[layer].challenges[data].rewardDisplay) ? (run(layers[layer].challenges[data].rewardDisplay, layers[layer].challenges[data])) : format(tmp[layer].challenges[data].rewardEffect)"></span></span>
			</span>
	...
	Vue.component('upgrade', {
		...
		<button v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!== undefined && tmp[layer].upgrades[data].unlocked" :id='"upgrade-" + layer + "-" + data' v-on:click="buyUpg(layer, data)" v-bind:class="{ [layer]: true, tooltipBox: true, upg: true, bought: hasUpgrade(layer, data), locked: (!(canAffordUpgrade(layer, data))&&!hasUpgrade(layer, data)), can: (canAffordUpgrade(layer, data)&&!hasUpgrade(layer, data))}"
			...
			<span v-else>
				...
				<span v-if="layers[layer].upgrades[data].effectDisplay"><br>效果: <span v-html="run(layers[layer].upgrades[data].effectDisplay, layers[layer].upgrades[data])"></span></span>
	...
	Vue.component('prestige-button', {
		...
		<button v-if="(tmp[layer].type !== 'none')" v-bind:class="{ [layer]: true, reset: true, locked: !tmp[layer].canReset, can: tmp[layer].canReset}"
			...
			v-html="(data?data:'重置') +' '+ prestigeButtonText(layer)" v-on:click="doReset(layer)">
		</button>
		`
	...
	Vue.component('resource-display', {
		...
		<div style="margin-top: -13px">
			...
			<br><br>
			<span v-if="tmp[layer].showBest">你至多拥有 {{formatWhole(player[layer].best)}} {{tmp[layer].resource}} <br></span>
			<span v-if="tmp[layer].showTotal">你总计获得了 {{formatWhole(player[layer].total)}} {{tmp[layer].resource}}<br></span>
		</div>
		`
	})
	...
	Vue.component('clickable', {
		...
		<button 
			...
			v-bind:class="{ sb1: tmp[layer].clickables[data].shake,upg: true, tooltipBox: true, can: tmp[layer].clickables[data].canClick, locked: !tmp[layer].clickables[data].canClick}"
		...
		<div v-if="tmp[layer].grid" class="upgTable">
			<div v-for="row in (data === undefined ? tmp[layer].grid.rows : data)" class="upgRow">
				<div v-for="col in tmp[layer].grid.cols">
					<div v-if="run(layers[layer].grid.getUnlocked, layers[layer].grid, row*100+col)"
						class="upgAlign" v-bind:style="{'margin': '-1px 0px',  'height': 'inherit',}">
	...
	Vue.component('achievement', {
		props: ['layer', 'data'],
		template: `
		<div v-if="tmp[layer].achievements && tmp[layer].achievements[data]!== undefined && tmp[layer].achievements[data].unlocked" v-bind:class="{ [layer]: true, achievement: true, tooltipBox:true, achlocked: !hasAchievement(layer, data), bought: hasAchievement(layer, data)}"
			...
			: (tmp[layer].achievements[data].goalTooltip ? tmp[layer].achievements[data].goalTooltip : (tmp[layer].achievements[data].tooltip ? tmp[layer].achievements[data].tooltip : '未完成'))
		"></tooltip>
