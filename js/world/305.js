addLayer("305", {
    ...
    tabFormat: {
        Ascension: {
            content: [
                ["display-text", function () {
                    return `You performed ${String(player[305].simulation)} simulation${player[305].simulation > 1 ? "s" : ""}.`
                }],
                ["display-text", function () {
                    return `Your highest ascension level is ${format(player[305].bestAscensions, 2)}, providing +${format(layers[305].getBestAscensionEffect(), 3)} base energy gain`
                }],
                "blank",
                ["clickables", "1"],
                "blank",
                ["display-text",
                    function () { return `Energy:${format(player[305].energy)}` }],
                "buyables",
                "blank",
                ["bar", "c1"]
            ],
        ...
        Charger: {
            content: [
                ["display-text", function () {
                    return `You have ${format(layers[305].getAvailableCharges(), 0)}/${format(getBuyableAmount(305, 21), 0)} unspent charge`
                }],
                ["blank", "50px"],
                ["clickables", [2]],
                ["clickables", [3, 4, 5]]
        ...
        Recursion: {
            content: [
                ...
                "blank",
                ["display-text", function () {
                    return `Micro Energy: ${format(player[305].microEnergy,
                        2)}<br><br>Highest Micro Energy: ${format(player[305].highestMicroEnergy)},<br>which boosts Energy production by x${format(player[305].highestMicroEnergy.pow(0.5).pow(layers[305].clickables[52].effect()).add(1), 2)}`
                }],
            ],
    ...
    getAvailableCharges() {
        let c = getBuyableAmount(305, 21); return c.sub(player[305].charge.reduce((p, s) => p + s, 0))
    },
    update(diff) {
        if (!getGridData('main',
            this.layer) || player.pause[this.layer]) return
    ...
    clickables: {
        ...
        21: {
            ...
            canClick: true,
            onClick() {
                layers[305].endSimulation(); player[305].charge = [0, 0, 0, 0, 0, 0, 0, 0, 0]
            },
            style() {
                return {
                    "min-height": "60px",
        ...
        31: {
            ...
            display() {
                return `Increase Energy gain.<br>-[Factor #1:Charge]<br>-[Factor #2:Energy]<br>-Effect: x${format(this.effect(),
                    2)}`
            },
            unlocked() { return player[305].bestAscensions.gte(5) },
            ...
            style() {
                return {
                    ...
                    "border": "5px solid",
                    "border-color": "#99cccc",
                    "background-color": (this.canClick() ? "#88bbbb" : "#336666")
                }
            },
        },
        41: {
            ...
            display() {
                return `Reduce the price of charges.<br>-[Factor #1:Charge]<br>-Effect: ^${format(this.effect(),
                    3)}`
            },
        ...
        42: {
            ...
            display() {
                return `Improve the effect of best ascension level.<br>-[Factor #1:Charge]<br>[Factor #2:Best Ascension Level]<br>-Effect: ^${format(this.effect(), 3)}`
            },
            unlocked() { return player[305].bestAscensions.gte(33) },
        ...
        51: {
            ...
            display() {
                return `Reduce the time speed slightly. Tick rate remains.<br>-[Factor #1:Charge]<br>-Effect: *${format(this.effect(), 4)}`
            },
        ...
        52: {
            ...
            display() {
                return `Improve the effect of Micro Energy.<br>-[Factor #1:Charge]<br>-Effect: ^${format(this.effect(), 1)}`
            },
        ...
        53: {
            ...
            display() {
                return `Enable Self-Replication for Micro Energy.<br>Warning: Replicates every tick.<br>-[Factor #1:Charge]<br>-Effect: *${format(this.effect(), 1)}/tick`
            },
            unlocked() { return player[305].bestAscensions.gte(1001) },
    ...
    buyables: {
        11: {
            title: "Ascend",
            display() {
                return `Your current Ascension Level is <h2 style="color:#121F24">${format(getBuyableAmount(305, this.id), 2)}</h2><br><br>Cost: ${format(this.cost())} Energy<br> Your ${format(getBuyableAmount(305, this.id), 2)} Ascension Levels multiplies your energy production by x${format(this.effect())}<br>`
            },
            ...
            buy() {
                if (player[305].bestAscensions.lt(33)) {
                    ...
                    setBuyableAmount(305,
                        this.id,
                        getBuyableAmount(305,
                            this.id).add(1))
                }
                else {
                    setBuyableAmount(305,
                        this.id,
                        player[305].energy.sub(1.1).max(0.5).log(3.9).add(1).floor().max(6))
                }
            },
        ...
        21: {
            title: "Gain a charge",
            display() {
                return `<h2>Requirement: <br>${format(this.cost(), 2)} Energy</h2>`
            },
            cost(x) {
                let y = x.add(1);
                return new Decimal(11).mul(x.pow(2).sub(23.8).max(1)).pow(y.pow(1.5).add(y).add(1)).pow(new Decimal(1.49).pow(y).sub(4).max(1)).pow(x.sub(6).max(1).pow(x.sub(6).pow(2))).pow(layers[305].clickables[41].effect())
            },
    ...
    microtabs: {
        reward: {
            "Highest Ascension Rewards": {
                content: [["display-text", function () {
                    let text = "";
                    for (i in a["Ascension"]) {
                        if (player[305].bestAscensions.gte(new Decimal(i))) {
                            text += `Ascension Level ${format(new Decimal(i), 0)}: ${a["Ascension"][i]}<br>`
                        } else {
                            text += `Next reward unlocks at Ascension Level ${format(new Decimal(i), 0)}`; break
                        }
                    } return text
                }]]
            }
    ...
    layerShown() {
        return getGridData('main',
            this.layer) && (!options.hideWorld || !player.world[this.layer]) && (!options[`line${Math.floor(this.layer / 100)}`])
    },
    hotkeys: [],
});
