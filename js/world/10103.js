let layer10103 = 20
let upgrades10103 = {}
let upgradesformat10103 = []
let upgradescost10103 = []
let milestones10103 = {}
let defined10103 = false

if (!(defined10103)) define10103()

function define10103() {
    defined10103 = true
    upgradescost10103 = getDelannoyMatrix(layer10103)
    ...
    for (let i = 0; i < 10; i++) {
        ...
        milestones10103[i + 30] = {
            ...
            完成了 ${formatPersent(player[this.layer].buy[0][i].div(this.target).clamp(0, 1))}
            ` },
        ...
        milestones10103[i + 40] = {
            ...
            目标是 ${formatWhole(this.target)}<br>
            完成了 ${formatPersent(player[this.layer].buy[0][i + 10].div(this.target).clamp(0, 1))}
            ` },
            done() { return this.base() == this.target },
            onComplete() { player[this.layer].milestone = player[this.layer].milestone.add(1) },
        ...
        milestones10103[i + 50] = {
            ...
            目标是 ${formatWhole(this.target)}<br>
            完成了 ${formatPersent(player[this.layer].buy[1][i].div(this.target).clamp(0, 1))}
            ` },
            done() { return this.base() == this.target },
            onComplete() { player[this.layer].milestone = player[this.layer].milestone.add(1) },
        ...
        milestones10103[i + 60] = {
            ...
            目标是 ${formatWhole(this.target)}<br>
            完成了 ${formatPersent(player[this.layer].buy[1][i + 10].div(this.target).clamp(0, 1))}
            ` },
            done() { return this.base() == this.target },
            onComplete() { player[this.layer].milestone = player[this.layer].milestone.add(1) },
